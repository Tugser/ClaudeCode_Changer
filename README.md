# Claude Code Provider Switch (macOS + Windows)

## Türkçe

### Amaç
Bu araçlar, Claude Code için sağlayıcı (Anthropic / GLM / MiniMax) ayarlarını macOS ve Windows üzerinde kolayca yönetmenizi sağlar. API anahtarlarını güvenli şekilde saklar ve Claude’un settings.json dosyasındaki env değerlerini güvenli şekilde günceller.
...
### Purpose
These tools help you manage Claude Code provider settings (Anthropic / GLM / MiniMax) on macOS and Windows. They store API keys securely and update env values in Claude’s settings.json safely.
